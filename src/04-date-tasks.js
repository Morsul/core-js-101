...
function timeSpanToString(startDate, endDate) {
  ...
  let s = '';
  s = `${dif.getUTCHours().toString().padStart(2, '0')}:`;
  s += `${dif.getMinutes().toString().padStart(2, '0')}:`;
  s += `${dif.getSeconds().toString().padStart(2, '0')}.`;
  s += `${dif.getMilliseconds().toString().padStart(3, '0')}`;
  return s;
}
 ...
 */
function angleBetweenClockHands(a) {
  let h = a.getUTCHours();
  if (h > 12) {
    h -= 12;
  }
  const min = a.getMinutes();
  let degree = Math.abs((h * 30 + min * 0.5) - (min * 6));
  if (degree > 180) {
